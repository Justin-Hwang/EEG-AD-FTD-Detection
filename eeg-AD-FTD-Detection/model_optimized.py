...
class ODCM(nn.Module):
    def __init__(self, in_channels, kernel_size, num_filters):
        ...
        self.conv3 = nn.Conv1d(in_channels=S*C, out_channels=S*C, kernel_size=kernel_size, padding=0, groups=S*C, bias=False)
        self.bn3 = nn.BatchNorm1d(S*C)
    ...
    def forward(self, x):  # x = [B = Batch, S = Channels, L = Time] , C = Filters
        ...
        z3buf = self.conv3(z2)
        z3buf = self.bn3(z3buf)
        z3buf = self.relu(z3buf) # z3 [B, S*C*C*C, L3]
...
class RTM(nn.Module):
    def __init__(self,
        ...
        self.token_dropout = nn.Dropout(p=0.3)
        ...
        self.blocks = nn.Sequential(*[TransformerBlock(D, num_heads, dropout=0.3) for _ in range(num_blocks)])
...
class STM(nn.Module):
    def __init__(self,
        ...
        self.token_dropout = nn.Dropout(0.3)

        # For each token, linear patch embedding D->D mapping
        self.token_embed = nn.Linear(D, D)
        self.cls_token   = nn.Parameter(torch.randn(1, 1, D))
        self.pos_embed   = nn.Parameter(torch.randn(1, 1 + S*C, D))
        self.blocks = nn.Sequential(*[TransformerBlock(dim=D, heads=num_heads, dropout=0.3) for _ in range(num_blocks)])

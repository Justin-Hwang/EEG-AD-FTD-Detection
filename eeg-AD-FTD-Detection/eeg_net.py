""" 
Define the EEGNet CNN model (designed for EEG spatial+temporal feature learning).

1. conv1 = Temporal convolution — learns time patterns (e.g., frequency bands).
2. depthwise_conv1 = Spatial convolution — learns how different brain regions relate.
3. separable_conv2 = Learns combined spatial-temporal features.
4. avgpool1/2 = Downsamples feature maps (reduces resolution).
5. dropout1/2 = Prevents overfitting.
6. dense + classifier = Final fully connected layers to output 3-class prediction.

It reshapes input to (batch, 1, channels, time) 
— like a grayscale image — and applies 2D convolutions.
